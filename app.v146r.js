// =============================================================
// v149_A (crítico)
// - Objetivos: restaurados + estructura robusta (evita error 'nac' undefined)
// - Persistencia: server como fuente de verdad; sin localStorage pesado
// - Analítica: carga CSV multi-año sin errores
// =============================================================
...
let STATE = {
  ...
  offers: [],
  orders: [],
  // Facturación: se persiste completo (incluye reclamadas y pago aprox.)
  billing: [],
  production: [],

  // Objetivos 2026 (estructura única)
  objectives: {
    data: {
      nac: { objTotal: 0, realTotal: 0 },
      exp: { objTotal: 0, realTotal: 0 },
      hot: { objTotal: 0, realTotal: 0 },
      car: { objTotal: 0, realTotal: 0 },
    },
    breakdown: { nac: [], exp: [], hot: [], car: [] },
  },
...
// Storage (v149_A): evitamos localStorage pesado para no disparar QuotaExceeded.
...
document.addEventListener('DOMContentLoaded', () => {
  (async () => {
    try{
      ...
      if(SERVER_ENABLED){
        ...
        if(serverState && typeof serverState === 'object'){
          ...
          // 1) Si el servidor es más nuevo (o local quedó vacío), restauramos servidor.
          // 2) Si el local es más nuevo, empujamos local al servidor.
          // 3) Si ambos existen pero falta "alguna sección" en uno de los dos, hacemos merge conservador.

          const serverHas = {
            orders: Array.isArray(serverState.orders) && serverState.orders.length>0,
            offers: Array.isArray(serverState.offers) && serverState.offers.length>0,
            billing: Array.isArray(serverState.billing) && serverState.billing.length>0,
            production: Array.isArray(serverState.production) && serverState.production.length>0,
            offersList: Array.isArray(serverState.__offers_list) && serverState.__offers_list.length>0,
            ordersList: Array.isArray(serverState.__orders_list) && serverState.__orders_list.length>0
          };
          const localHas = {
            orders: Array.isArray(STATE.orders) && STATE.orders.length>0,
            offers: Array.isArray(STATE.offers) && STATE.offers.length>0,
            billing: Array.isArray(STATE.billing) && STATE.billing.length>0,
            production: Array.isArray(STATE.production) && STATE.production.length>0,
            offersList: Array.isArray(STATE.__offers_list) && STATE.__offers_list.length>0,
            ordersList: Array.isArray(STATE.__orders_list) && STATE.__orders_list.length>0
          };
    ...
    // objectives
    if(!STATE.objectives || typeof STATE.objectives !== 'object') {
        STATE.objectives = { data: { nac:{objTotal:0,realTotal:0}, exp:{objTotal:0,realTotal:0}, hot:{objTotal:0,realTotal:0}, car:{objTotal:0,realTotal:0} }, breakdown: { nac:[], exp:[], hot:[], car:[] } };
    }
    if(!STATE.objectives.data) STATE.objectives.data = { nac:{objTotal:0,realTotal:0}, exp:{objTotal:0,realTotal:0}, hot:{objTotal:0,realTotal:0}, car:{objTotal:0,realTotal:0} };
    if(!STATE.objectives.breakdown) STATE.objectives.breakdown = { nac:[], exp:[], hot:[], car:[] };
            ...
            // Merge conservador: si el servidor trae algo que el local no tiene, lo copiamos al local.
            let merged = false;
            const copyIfMissing = (localKey, serverKey) => {
              if(!localHas[localKey] && serverHas[localKey]){ STATE[serverKey] = serverState[serverKey]; merged = true; }
            };
            copyIfMissing('orders','orders');
            copyIfMissing('offers','offers');
            copyIfMissing('billing','billing');
            copyIfMissing('production','production');
            copyIfMissing('offersList','__offers_list');
            copyIfMissing('ordersList','__orders_list');

            // Si el local tiene más que el servidor (o el servidor se quedó a 0), subimos local.
            const localNewerOrRicher = (localStamp >= serverStamp) && (
              (localHas.orders && !serverHas.orders) ||
              (localHas.offers && !serverHas.offers) ||
              (localHas.billing && !serverHas.billing) ||
              (localHas.production && !serverHas.production) ||
              (localHas.offersList && !serverHas.offersList) ||
              (localHas.ordersList && !serverHas.ordersList)
            );
      ...
      updateServerStatusUI();
      // Ping periódico para que el indicador muestre hora real (y detecte caídas)
      if(SERVER_ENABLED){
        try{ serverPing(); }catch(e){}
        try{ setInterval(()=>{ try{ _serverAvailable = null; serverPing(); }catch(e){} }, 30000); }catch(e){}
      }
      initMocks();
      initHandlers();
      initNavBindings();
...
if (menuImportServer) menuImportServer.onclick = () => {
    ...
    if (restoreInput) restoreInput.onchange = async (e) => {
        ...
        try {
            ...
            else if (obj) STATE = obj;            // Hardening: si el backup no incluye objectives, los inicializamos para evitar crashes.
            if(!STATE.objectives || typeof STATE.objectives !== 'object') STATE.objectives = {};
            if(!STATE.objectives.data) STATE.objectives.data = {};
            if(!STATE.objectives.data.nac) STATE.objectives.data.nac = { objTotal: 0, realTotal: 0 };
            if(!STATE.objectives.data.exp) STATE.objectives.data.exp = { objTotal: 0, realTotal: 0 };
            if(!STATE.objectives.data.hot) STATE.objectives.data.hot = { objTotal: 0, realTotal: 0 };
            if(!STATE.objectives.data.car) STATE.objectives.data.car = { objTotal: 0, realTotal: 0 };
            if(!STATE.objectives.breakdown) STATE.objectives.breakdown = { nac: [], exp: [], hot: [], car: [] };
...
function restoreState() {
    ...
    const s = USE_LOCAL_CACHE ? localStorage.getItem('cch_v55_db') : null;
    if(s){
        try{
            STATE = JSON.parse(s);
        }catch(err){
            ...
            STATE = {
                ...
                billing: [],
                production: [],
                objectives: { data: { nac:{objTotal:0,realTotal:0}, exp:{objTotal:0,realTotal:0}, hot:{objTotal:0,realTotal:0}, car:{objTotal:0,realTotal:0} }, breakdown: { nac:[], exp:[], hot:[], car:[] } },
                analytics: { ordersByYear: {}, offersByYear: {}, mode: 'orders' }
            };
        }
    }
...
function parseOrdersDetailed(cleanCsvText){
    ...
    for(const r of rowsObj){
        ...
        const comercial = g('Comercial','Agente','Vendedor');
...
function updateUI() {
    ...
    ['offers','orders'].forEach(t => {
        ...
        if(list){
            if(arr.length){
                const PAGE_SIZE = 30;
                window.__homePage = window.__homePage || { offers: 1, orders: 1 };
                const total = arr.length;
                const pages = Math.max(1, Math.ceil(total / PAGE_SIZE));
                const page = Math.min(Math.max(1, (window.__homePage[t]||1)), pages);
                const fromIdx = (page-1)*PAGE_SIZE;
                const slice = arr.slice(fromIdx, fromIdx + PAGE_SIZE);
                const from = fromIdx + 1;
                const to = Math.min(total, fromIdx + PAGE_SIZE);
                const pager = `
                  <div class="mini-pager">
                    <div class="mini-pager__meta">Mostrando <b>${from}-${to}</b> de <b>${total}</b></div>
                    <div class="mini-pager__btns">
                      <button class="btn btn-ghost" ${page<=1?'disabled':''} data-pager="prev">◀</button>
                      <span class="mini-pager__page">${page}/${pages}</span>
                      <button class="btn btn-ghost" ${page>=pages?'disabled':''} data-pager="next">▶</button>
                    </div>
                  </div>`;

                list.innerHTML = pager + slice.map((item, i) => {
                  const realIdx = fromIdx + i;
                  return `<div class="item-card" onclick="openDetail('${t}', ${realIdx})"><div class="item-tit"><span>${item.client}</span> <strong>${formatMoney(item.amount)}</strong></div><div class="item-desc">${item.desc||'-'}</div></div>`;
                }).join('');

                const prev = list.querySelector('[data-pager="prev"]');
                const next = list.querySelector('[data-pager="next"]');
                if(prev) prev.onclick = (e)=>{ e.stopPropagation(); window.__homePage[t]=Math.max(1,page-1); updateUI(); };
                if(next) next.onclick = (e)=>{ e.stopPropagation(); window.__homePage[t]=Math.min(pages,page+1); updateUI(); };
            } else {
        ...
        file.onchange = async () => {
            ...
            try{
                for (const f of files){
                    ...
                    const rows = (mode==='offers') ? await readOffersFile(f) : await readOrdersFile(f);                    // Soporta CSV multi-año: si el fichero contiene varios años, los separa automáticamente.
                    const getYearFromRow = (row) => {
                        if(!row || typeof row !== 'object') return '';
                        const keys = Object.keys(row);
                        const yearKey = keys.find(k => /^(año|ano|year|ejercicio)$/i.test(String(k).trim()))
                                      || keys.find(k => /año|ano|year|ejercicio/i.test(String(k)));
                        let y = '';
                        if(yearKey && row[yearKey] != null){
                            const mm = String(row[yearKey]).match(/(19|20)\d{2}/);
                            if(mm) y = mm[0];
                        }
                        if(!y){
                            // intenta derivar desde fecha
                            const dateKey = keys.find(k => /fecha/i.test(String(k)));
                            if(dateKey && row[dateKey]){
                                const s = String(row[dateKey]);
                                const mm = s.match(/(19|20)\d{2}/);
                                if(mm) y = mm[0];
                            }
                        }
                        return y;
                    ...
                    const forced = (forcedYear && /^\d{4}$/.test(forcedYear)) ? forcedYear : '';
                    const modeBucket = (mode==='offers') ? STATE.analytics.offersByYear : STATE.analytics.ordersByYear;

                    if(!forced && years.size > 1){
                        // Multi-año real
                        for (const y of Array.from(years).sort()){
                            const subset = rows.filter(r => getYearFromRow(r) === y);
                            const norm = (mode==='offers') ? normalizeOffers(subset, y) : normalizeOrders(subset, y);
                            const existing = modeBucket[y];
                            modeBucket[y] = mergeNormalizedOrders(existing, norm);
                        }
                    } else {
                        ...
                        const existing = modeBucket[detectedYear];
                        modeBucket[detectedYear] = mergeNormalizedOrders(existing, norm);
                    }
                }
                saveState();
                renderAnalytics();
